 ...
 * @param spec `{ options|noul|score, theta?, edges?, letterPrior?, prompt?, request?, ms?, model? }`
 */
function readResponse( resp, spec ) {
	...
	const dist = readout.distribution(entries, letters, s.letterPrior);
	...
	const out = {
		...
		/** The letter prior APPLIED to this decision (null when none) — an audited correction is a
		 *  PUBLISHED one: it belongs next to p1, like `T` in a calibrated decision. */
		letterPrior  : dist.letterPrior,
...
function createClient( options ) {
	...
	/* The letter prior, like theta, is a caller-decided layer — but a JSON array does not fit an env
	 * var the way a number does, so a malformed NOTJEV_LETTER_PRIOR is REFUSED, not guessed. */
	const letterPrior0 = o.letterPrior !== undefined ? o.letterPrior
		: (env.NOTJEV_LETTER_PRIOR !== undefined && env.NOTJEV_LETTER_PRIOR !== '')
			? (() => { try { return JSON.parse(env.NOTJEV_LETTER_PRIOR); }
				catch ( e ) { throw fail('NOTJEV_BAD_PRIOR', 'notjev: NOTJEV_LETTER_PRIOR is not JSON '
					+ '(a number array, as estimated by harness.letterPrior): ' + env.NOTJEV_LETTER_PRIOR); } })()
			: null;
	...
	async function decide( q ) {
		...
		return readResponse(r.resp, {
			...
			letterPrior: (q && q.letterPrior !== undefined && q.letterPrior !== null) ? q.letterPrior : letterPrior0,
			prompt : b.content,
			request: b.params,
			ms     : Date.now() - t0,
	...
	return {
		baseUrl: baseUrl, model: model, theta: theta0, path: path, letterPrior: letterPrior0,
