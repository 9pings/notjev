...
function createGateway(options) {
	...
	const maxConcurrentUpstream = positive(options.maxConcurrentUpstream, 64, 'maxConcurrentUpstream');
	...
	return http.createServer(async (req, res) => {
		...
		try {
			...
			if (upstreamInFlight >= maxConcurrentUpstream)
				return send(res, 429, { error: { code: 'NOTJEV_RATE_LIMITED', message: 'Too many concurrent upstream requests' } });
			upstreamInFlight++; countedUpstream = true;
		...
		} finally { clearTimeout(timer); if (countedUpstream) upstreamInFlight--; }
	});
}
