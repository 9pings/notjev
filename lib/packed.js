 ...
 * @param opts `{ theta?, edges?, letterPrior? }` — `letterPrior` : tableau (contrôlé par slot, la
 *        longueur doit être celle du menu) ou fonction `(letters) => prior`, pour des slots de
 *        tailles différentes.
 */
function readPacked( resp, packed, opts ) {
	...
	const priorOf = opts && opts.letterPrior;
	const priorFor = typeof priorOf === 'function' ? priorOf : () => priorOf;
	return packed.slots.map(( s, i ) => {
		const pos = packed.positions[i];
		const entries = entriesAt(pl, pos) || [];
		const dist = readout.distribution(entries, s.letters, priorFor(s.letters));
...
/**
 * LE CLIENT PACKED — `baseUrl` a la MÊME forme que dans le cœur (`createClient`) : la racine du serveur,
 * avec ou sans `/v1` (mesuré 21/09 en vol : « doit finir par /v1 » contredisait le cœur, et un appelant qui
 * passait la même valeur aux deux obtenait `/v1/v1/...` → 404). POST `<racine>/v1/completions`.
 * `tokenize` est injecté : c'est le tokeniseur DU SERVEUR qui place les positions.
 */
function createPackedClient( o ) {
	const f = (o && o.fetch) || globalThis.fetch;
	const base = String((o && o.baseUrl) || '').replace(/\/$/, '').replace(/\/v1$/, '') + '/v1';
	return {
