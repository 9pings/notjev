 ...
 * auditable rather than assumed — measured at <= 0.08 % of coverage on the bench.
 ...
 *
 * @returns `{ probabilities, mass, coverage, exactMass, spacedMass, degraded }` —
 ...
 *          a silent uniform would read as hesitation.
 */
function distribution( entries, letters ) {
	...
	return {
		probabilities: degraded ? mass.map(() => 1 / n ) : mass.map(( x ) => x / coverage ),
		mass         : mass,
		coverage     : coverage,
